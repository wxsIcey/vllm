...
class PlainDraftModelSpeculator(DraftModelSpeculator):
    ...
    def sample_draft(
        ...
        """Greedy argmax or Gumbel-max sampling with optional logit storage."""
        logits = self.model.compute_logits(hidden_states)  # type: ignore[union-attr]
    ...
    def _prefill(
        ...
        if skip_attn:
            ...
            hidden_states = self._run_model(
                src_tokens, None, None, num_tokens_across_dp
            )
            positions_buf = self.input_buffers.positions
        else:
            ...
            with set_forward_context(
                ...
                cudagraph_runtime_mode=CUDAGraphMode.NONE,
                num_tokens_across_dp=num_tokens_across_dp,
                slot_mapping=prefill_slot_maps_by_layer,
                batch_descriptor=batch_descriptor,
            ):
                hidden_states = self.model(  # type: ignore[misc]
